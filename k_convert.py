...
DIR_to_SD_SAFETY_CHECKER = "CompVis/stable-diffusion-safety-checker"
DIR_to_OPEN_VIT_LARGE_PATCH14 = "openai/clip-vit-large-patch14"
...
def convert_ldm_unet_checkpoint(checkpoint, config, extract_ema=False):
    ...
    if "input_blocks.0.0.bias" in unet_state_dict:
      new_checkpoint["conv_in.bias"] = unet_state_dict["input_blocks.0.0.bias"]

    if "out.0.weight" in unet_state_dict:
      new_checkpoint["conv_norm_out.weight"] = unet_state_dict["out.0.weight"]
    if "out.0.bias" in unet_state_dict:
      new_checkpoint["conv_norm_out.bias"] = unet_state_dict["out.0.bias"]
    if "out.2.weight" in unet_state_dict:
      new_checkpoint["conv_out.weight"] = unet_state_dict["out.2.weight"]
    if "out.2.bias" in unet_state_dict:
    ...
    for i in range(num_output_blocks):
        ...
        if len(output_block_list) > 1:
            ...
            conv_state_keys = [["conv.weight", "conv.bias"], ["conv.bias", "conv.weight"]]
            for conv_state_key in conv_state_keys:
                if conv_state_key in output_block_list.values():
...
def convert_ldm_clip_checkpoint(checkpoint):
    text_model = CLIPTextModel.from_pretrained(pretrained_model_name_or_path=DIR_to_OPEN_VIT_LARGE_PATCH14)
...
def convert_full_checkpoint(
    ...
    if vae_pt_path:
        ...
        for key, value in vae_checkpoint.items():
            new_key = 'first_stage_model.{}'.format(key)
            if new_key in checkpoint and checkpoint[new_key].shape == value.shape:
    ...
    tokenizer = CLIPTokenizer.from_pretrained(pretrained_model_name_or_path=DIR_to_OPEN_VIT_LARGE_PATCH14)
...
def convert_full_checkpoint_r2(
        checkpoint_pathlist,
        config_file,
        scheduler_type,
        extract_ema,
    ...
    for checkpoint_path in checkpoint_pathlist:
      ...
      with safe_open(checkpoint_path, framework="pt", device=fdevice) as f:#device=0) as f:
        for k in f.keys():
            checkpoint[k] = f.get_tensor(k)
...
def convert_full_checkpoint_unet(
        checkpoint_path: str,
        config_file,
        scheduler_type,
        extract_ema,
...
safe_tensor_path="./xperoEnd1essModel_v1.safetensors"
